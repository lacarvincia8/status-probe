...
```
status-probe [flags] <url>...
```

Each positional argument is a URL. If you pass several, `status-probe`
...
```bash
status-probe --expect 200 --timeout 5s https://prod.example.com/healthz
case $? in
  0)   echo "OK" ;;
